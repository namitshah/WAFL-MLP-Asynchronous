...
# Configuration
#
output_code = "line"  # name of the file
exp_codes = [
    "line",
    #"rwp1000",
    #"rwp2000",
...
max_epoch = 100  # default 5000
n_device = 4  # fixed to 10
...
def main():
    ...
    #
    x = [(i + 1) * 10 for i in range(max_epoch)]
    acc = [
    ...
    #
    for input_epoch in range(1, max_epoch + 1):
        epoch = input_epoch * 10
        print(epoch)
        ...
        for e in range(len(exp_codes)):
            ...
            acc[e][input_epoch - 1] = statistics.mean(accuracies)

    # Save the results in a Graph
    save_accuracy_trend(
        x, acc, exp_codes, title=f"QUAD LINE TRAINING ACTUAL @ Namit's MB", save_path=f"../accuracy/accuracy_{output_code}.png"
    )
